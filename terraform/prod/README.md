# Fintech prod infrastructure - vuln-bank

이 prod 환경은 기존 JOA 서비스를 제거하고, Commando-X/vuln-bank를 public fintech security testing 서비스로 배포하는 기준입니다.
...
- RDS: `fin-prod-db`, PostgreSQL 16, DB name `vulnerable_bank`
- WAF: `fin-prod-waf`
...
- MFA enforcement policy: `fin-prod-deny-without-mfa`
- CloudTrail: `fin-prod-cloudtrail`
- CloudTrail logs: S3 `fin-prod-cloudtrail-364585378962-ap-northeast-2`, CloudWatch Logs `/aws/cloudtrail/fin-prod-cloudtrail`
