...
이 문서는 비용 방지를 위해 prod 환경을 내리는 순서입니다.

기준 서비스: `Commando-X/vuln-bank`
리전: `ap-northeast-2`
AWS Account: `364585378962`
Terraform 폴더: `C:\Users\polis\OneDrive\Desktop\aws-cloud-security-fintech-platform-main\terraform\prod`
...
aws rds describe-db-instances --region ap-northeast-2 --query "DBInstances[?DBInstanceIdentifier=='fin-prod-db'].DBInstanceStatus"
aws ec2 describe-nat-gateways --region ap-northeast-2 --filter "Name=tag:Name,Values=fin-prod-nat" --query "NatGateways[].State"
...
aws cloudtrail describe-trails --region ap-northeast-2 --trail-name-list fin-prod-cloudtrail
aws s3api head-bucket --bucket fin-prod-cloudtrail-364585378962-ap-northeast-2
```
