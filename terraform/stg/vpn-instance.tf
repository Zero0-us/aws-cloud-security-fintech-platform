...
# 2. VPN용 EIP (Terraform에서 생성)
resource "aws_eip" "vpn_fixed" {
  domain = "vpc"

  tags = {
    Name = "fin-stg-vpn-eip-fixed"
  }
}
...
resource "aws_instance" "vpn" {
  ...
  subnet_id                   = aws_subnet.stg_pub_2a.id
