...
Prod / Dev / Stage 계정은 각각 분리된 로그 버킷으로 로그를 보낸다. SOC 계정 자체 로그는 `fin-athena-result-s3`의 `soc-logs/` prefix로 보낸다.
...
SOC Terraform은 CloudTrail, AWS Config, SOC VPC Flow Logs를 직접 활성화하고 `fin-athena-result-s3/soc-logs/`로 적재한다. ALB, WAF, EKS audit logs는 실제 서비스 리소스가 있는 Production / Development / Staging 계정에서 각 계정용 로그 버킷으로 destination을 지정해야 하며, 이 Terraform은 `baseline/service-log-intake-manifest.json` 산출물을 준비한다.
