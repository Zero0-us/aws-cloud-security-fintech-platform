...
Development 계정 로그 저장소다.

저장 대상:

- Development CloudTrail
- Development VPC Flow Logs
- Development AWS Config delivery
- Development WAF Logs
- Development ALB Access Logs
...
```text
s3://fin-athena-results/soc-logs/cloudtrail/
s3://fin-athena-results/soc-logs/config/
s3://fin-athena-results/soc-logs/vpc-flow-logs/
```

기본 Workgroup 결과 위치:

```text
s3://fin-athena-results/athena-results/sc-audit/
```

Compliance 산출물 위치:

```text
s3://fin-athena-results/baseline/
s3://fin-athena-results/monthly-audit/
```
...
- Staging 접근기록은 `fin-stage-log-s3`에 저장한다.
- SOC 접근기록은 `fin-athena-results/soc-logs/`에 저장한다.
...
- Staging 계정 접근기록: `fin-stage-log-s3`에 적재한다.
- SOC 계정 접근기록: `fin-athena-results/soc-logs/cloudtrail/`에 적재한다.
...
Prod / Dev / Stage 계정은 각각 분리된 로그 버킷으로 로그를 보낸다. SOC 계정 자체 로그는 `fin-athena-results`의 `soc-logs/` prefix로 보낸다.
...
Stage Logs → fin-stage-log-s3
SOC Logs  → fin-athena-results/soc-logs/
→ Glue Catalog / Athena
→ fin-athena-results/athena-results/
```

SOC Terraform은 CloudTrail, AWS Config, SOC VPC Flow Logs를 직접 활성화하고 `fin-athena-results/soc-logs/`로 적재한다. ALB, WAF, EKS audit logs는 실제 서비스 리소스가 있는 Production / Development / Staging 계정에서 각 계정용 로그 버킷으로 destination을 지정해야 하며, 이 Terraform은 `baseline/service-log-intake-manifest.json` 산출물을 준비한다.
...
ISMS-P 대응을 위해 `baseline/isms-p-control-mapping.json`을 `fin-athena-results` bucket에 저장한다. 이 파일은 ISMS-P 기반 통제 항목, 증적 위치, 연결된 AWS Config Rule을 정의하며 월간 감사 Lambda 보고서에 포함된다.
...
- 보고서를 `fin-athena-results` bucket의 `monthly-audit/` prefix에 저장
- SOC 감사 SNS Topic으로 보고서 S3 위치와 요약 결과 발송
- Athena named query로 월간 CloudTrail 활동 요약 쿼리 제공
- 보고서 보관 위치로 `fin-athena-results` bucket 사용
...
- Production / Development / Staging의 ALB, WAF, EKS 로그는 각 계정의 서비스 리소스에서 SOC audit bucket으로 destination을 설정해야 한다.
